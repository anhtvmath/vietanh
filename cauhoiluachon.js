...
"\\begin{tikzpicture}[thick,xscale=0.95,yscale=0.5, every node/.style={font=\\fontsize{18}{20}\\selectfont}]\n" + 
"\\def\\a{8}\n" + 
"\\def\\b{7}\n" + 
...
var debai = "Cho hàm số $y=f(x)$ có bảng biến thiên như sau:\n" +
            "\\begin{tikzpicture}[thick,xscale=1.4,yscale=0.9, every node/.style={font=\\large}]\n" + 
            "\\def\\a{10}\n" +
            "\\def\\b{7}\n" +
            "\\draw (-0.5,0.5) rectangle (\\a-0.5,-\\b+0.5);\n" + 
            "\\draw (-0.5,-0.5)--(\\a-0.5,-0.5);\n" +
